...
class Pois_tesp extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->fapp->addCSS(base_url('/assets/css/pois_tesp.css'));
		$this->fapp->addCSS(base_url("/assets/plugins/select2/select2.min.css"),false);
		$this->fapp->addJS(base_url("/assets/js/custom/pois_tesp.js"),false);
    ...
    public function index($type="one")	{
		...
		$data['type'] = $type;
		$data['progressbar'] = pois_tesp_steps_pregressbar(1);
		...
		$this->fapp->show('pois_tesp', 'pois_tesp_form_step', $data);
	}
	
	public function pois_tesp_form_submit()	{
        ...
        if($this->input->is_ajax_request()){
			...
			{
				$data['progressbar'] = pois_tesp_steps_pregressbar($step);
				...
				if(!empty($this->session->userdata('personal_details'))){
					...
					$data['personal_details_view'] = $this->load->view('pois_tesp/summary_personal_details', $data, true);
					...
					$data['view_payment_opt'] = $this->load->view('pois_tesp/view_payment_options', $data, true);
				}
				...
				if(!empty($this->session->userdata('payment_options'))){
					...
					$data['payment_options'] = array_merge($payment_options_blank, $this->session->userdata('payment_options'));
					$data['payment_options_view'] = $this->load->view('pois_tesp/summary_payment_options', $data, true);
				}
				...
				if(!empty($this->session->userdata('plan_details'))){
					...
					$data['plan_details_view'] = $this->load->view('pois_tesp/summary_plan_details', $data, true);
				}
	...
	public function edit_payment()	{
		if($this->input->is_ajax_request()){
			...
			$data['type'] = "topup";
			$view = $this->load->view('pois_tesp/edit_list_' . $page, $data, true);
	...
	public function submit_application()	{
		if($this->input->is_ajax_request()){
			...
			/*$form_forester_id = POIS_TESP_CUSTOMER_APPLICATION_PFX . date("dm") . $new_constant_value;*/
			...
			if(!empty($plan_details))
			{
				$customer_pdf .= '<h1>Plan Details</h1>
										Monthly Contribution Amount: &pound;'.@number_format(POIS_TESP_MONTHLY_AMOUNT,'2','.',',').'<br>
										';
			...
			if(!empty($payment_options))
			{
				if(isset($payment_options['choose_payment_option_monthly']) && $payment_options['choose_payment_option_monthly'] == 1)
				{
					...
					$customer_xml .= '
										...
										<MonthlyContributionAmount>£'.@number_format(POIS_TESP_MONTHLY_AMOUNT,'0','',',').'</MonthlyContributionAmount>
			...
			$customer_xml .= '</Form>';
			$filename_xml = POIS_TESP_CUSTOMER_APPLICATION_PFX . $new_constant_value . '-' .date("d-m-Y").'-'. str_replace(" ","-",$personal_details['last_name']) .'.xml';
			$filename_pdf = POIS_TESP_CUSTOMER_APPLICATION_PFX . $new_constant_value . '-' .date("d-m-Y").'-'. str_replace(" ","-",$personal_details['last_name']) .'.pdf';
			
			$xml_file_path = generate_xml($customer_xml, 'POIS TESP', $filename_xml);
			$pdf_file_path = generate_pdf($customer_pdf, 'Tax Exempt Savings Plan Customer PDF', $filename_pdf, $pdf_password);
				...
				$body['pdf_link'] = base_url('pois_tesp/download/'.$filename_pdf);
				$body['admin_body_content'] = POIS_TESP_ADMIN_EMAIL_BODY_CONTENT;
				
				sendEmailtoCustomer($personal_details['email'], $body, "Your Tax Exempt Savings Plan application has been submitted", false, "pois_tesp_customer_confirmation");
				
				sendEmailtoPOISAdmin(ADMIN_EMAIL, $body, "New online product application(TESP)");
